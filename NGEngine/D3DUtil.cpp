...
Microsoft::WRL::ComPtr<ID3D12Resource> D3DUtil::CreateDefaultBuffer(ID3D12Device* device, ID3D12GraphicsCommandList* cmdList, const void* initData, UINT64 byteSize, Microsoft::WRL::ComPtr<ID3D12Resource>& uploadBuffer)
{
	...
	// In order to copy CPU memory data into our default buffer, we need to create a intermediate upload heap.
	ThrowIfFailed(device->CreateCommittedResource(
	...
	// will copy the cpu memory into the intermediate upload heap. then, using ID#D12CommandList::CopySubresourceRegion,
